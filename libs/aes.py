...
if __name__ == "__main__":
	...
	key = "7A82C30ADDD5182DAC887DCEBDF1276FB294E512439C5DE0EB6D30835DDC3858"
	key = [int("%s%s" % (key[i], key[i+1]), 16) for i in range(0,len(key),2)]
	iv = "EAA7FF5A690C2E465CFB5958FE4B3844"
	iv = [int("%s%s" % (iv[i], iv[i+1]), 16) for i in range(0,len(iv),2)]
	mode,orig_len,ciph = moo.encrypt("This is a test text",moo.modeOfOperation["OFB"],key,moo.aes.keySize["SIZE_256"],iv)
	print ciph
	decr = moo.decrypt(ciph,orig_len,mode,key,moo.aes.keySize["SIZE_256"],iv)
	print decr